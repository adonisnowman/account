...
if (!defined('ROOT_PATH') ) 
{
	...
	if( file_exists("/home/cfd888/public_html/swoole.bestaup.com"))
	define('ROOT_PATH', "/home/cfd888/public_html/swoole.bestaup.com" . $dir);
	else define('ROOT_PATH', "/var/www" . $dir);
...
$container->set('AdminDatabase', function () use ($config) {
	return new \Phalcon\Db\Adapter\Pdo\Mysql(array(
		"host" => $config->AdminDatabase->host,
		"username" => $config->AdminDatabase->username,
		"password" => $config->AdminDatabase->password,
		"dbname" => $config->AdminDatabase->name,
		'options' => array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES ' . $config->AdminDatabase->charset)
	));
});
